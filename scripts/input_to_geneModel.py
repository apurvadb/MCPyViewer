...
def parse_args():
    parser = argparse.ArgumentParser(description="MCPyV gene model")
    parser.add_argument(
        "-i",
        "--integration_data",
        required=True,
    ...
    parser.add_argument(
        ...
        "--ref",
        required=True,
        help="Path to the reference (Human+MCPV) genome file",
    )
    parser.add_argument(
        "-o", "--output", required=True, help="Output directory for results"
    )
    parser.add_argument(
        "-s", "--sample_list", nargs="+", required=True, help="List of sample names"
    )
...
def main():
    ...
    integration_data = args.integration_data
    gtf_file = args.gtf
    ...
    MCV_dic = {
        ...
        "small_T": [[4827, 5387]]
#        "intergenic": [[1, 464], [2428, 2502], [4723, 4826]]
    }
    ...
    for sample in sample_list:
        path = os.path.join(integration_data, f"Sample_{sample}/call_fusion_virus")
    ...
    for sample in siteDic:
        for ins in siteDic[sample]:
            ...
            for gene in geneDic:
                if (
                    geneDic[gene][1] == chro
                    and int(geneDic[gene][2]) <= int(site)
                    and int(geneDic[gene][3]) > int(site)
                ):
    ...
    countDic = dict(
        pd.Series(
            [gene for sample in geneSiteList for gene in sample[1]]
        ).value_counts()
    )
    ...
    genePlotDf = pd.DataFrame(
        genePlotDfList,
        columns=["sample", "gene", "chr", "ins", "start", "end", "direction"],
    )
    ...
    with open(os.path.join(output_dir, "extractContig.sh"), "w") as scriptfile:
        for each in genePlotDf.itertuples():
            ...
            sample = each.sample
            gene = each.gene
            contigfile = os.path.join(
                integration_data,
                f"Sample_{sample}/call_fusion_virus/ContigsSequence.fa",
            )
            resfile = os.path.join(integration_data, f"Sample_{sample}/{gene}.fa")
    ...
    with open(os.path.join(output_dir, "alignContig.sh"), "w") as scriptfile:
        ...
        for each in genePlotDf.itertuples():
            ...
            sample = each.sample
            resfile = os.path.join(integration_data, f"Sample_{sample}/{gene}.fa")
            ref = os.path.join(integration_data, ref_hum_mcpv)
            scriptfile.write(
                f"""bwa mem -R '@RG\\tID:mcv\\tSM:mcv\\tLB:mcv\\tPL:ILLUMINA' -M -t 8 {ref} {resfile} > {os.path.join(integration_data, f'Sample_{sample}/{gene}.sam')};
    samtools view -bhS {os.path.join(integration_data, f'Sample_{sample}/{gene}.sam')} > {os.path.join(integration_data, f'Sample_{sample}/{gene}.bam')};
    samtools sort -@ 8 -o {os.path.join(integration_data, f'Sample_{sample}/{gene}.sort.bam')} {os.path.join(integration_data, f'Sample_{sample}/{gene}.bam')};
    samtools index {os.path.join(integration_data, f'Sample_{sample}/{gene}.sort.bam')};"""
            )
    ...
    for eachline in genePlotDfList:
        ...
        bam = f"{path}/Sample_{sample}/{gene}.sort.bam"
        chro = eachline[2]
        ...
        for read in samfile.fetch(chro, int(genestart), int(geneend)):
            ...
            for eachMCVgene in MCV_dic:
                for eachSlot in MCV_dic[eachMCVgene]:
                    if int(mcvSite) >= int(eachSlot[0]) and int(mcvSite) <= int(
                        eachSlot[1]
                    ):
                    ...
                    if supchro == "NC_010277.2":
                        ...
                        for each in MCV_dic:
                            for eachone in MCV_dic[each]:
                                if int(suppos) >= int(eachone[0]) and int(
                                    suppos
                                ) <= int(eachone[1]):
                                    each1 = each
                                    newFlag.append(each1)
                                    hasEach1 = True
                                if int(suppos) + match >= int(eachone[0]) and int(
                                    suppos
                                ) + match <= int(eachone[1]):
                                    each2 = each
                        ...
                        if slotCigars[0] == "M":
                            if (
                                supstrand == strand
                            ):  # read converted to consistent cigar to original human alignment
                                if (
                                    infoList[6] == "+"
                                ):  # read on hum refence was not converted
                                    pointStart = 0
                                ...
                                if (
                                    infoList[6] == "+"
                                ):  # read on hum refence was not converted, converted to human alignments
                                    pointStart = fullLength
                                    pointEnd = fullLength - int(slots[0])
                            ...
                            if (
                                ...
                                if (
                                    infoList[6] == "-"
                                ):  # read on hum refence was converted
            ...
            annotation.append(
                            [sample, gene, sample, contig, chro, start, length, strand, contigins]
                        )
            # print(inserts)
