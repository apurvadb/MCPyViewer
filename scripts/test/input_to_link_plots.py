...
def parse_args():
    parser = argparse.ArgumentParser(description="Process and analyze MCPyV integrations and generate plots.")
    parser.add_argument('-i', '--integration_data', required=True, help='Path to the searcHPV results directory')
    parser.add_argument('-s', '--samples_file', required=True, help='Path to the sample txt file')
...
def generate_link_data(mcv_info, integration_data):
    score_list = []
...
def generate_bed_files(mcv_info, integration_data, output_dir):
    found_bam = None
    
    # Iterate over the samples to find an available BAM file.
    for sample in mcv_info['Sample']:
        if str(sample) != 'nan':
            sample_path = os.path.join(integration_data, f'Sample_{sample}/alignment/alignment.RG.indelre.mkdup.sort.bam')
...
def main():
    ...
    # Load data
    mcv_info = pd.read_csv(args.samples_file, sep = "\t")
    all_dic = read_data(f'{args.integration_data}')
    ...
    score_list = generate_link_data(mcv_info, args.integration_data)
    ...
    generate_bed_files(mcv_info, args.integration_data, args.output_dir)
