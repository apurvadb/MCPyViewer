...
usage() {
    echo "Usage: $0 -i integration_data -t transcript_gtf -e exon_gtf -r ref_fa -d output_dir -f file_to_process sample1 sample2 ... sampleN"
    exit 1
}
...
while getopts ":i:t:e:r:d:f:" opt; do
  case $opt in
    i) INTEGRATION_DATA="$OPTARG";;
...
if [ -z "$INTEGRATION_DATA" ] || [ -z "$TRANSCRIPT_GTF" ] || [ -z "$EXON_GTF" ] || [ -z "$REF_FA" ] || [ -z "$OUTPUT_DIR" ] || [ -z "$FILE_TO_PROCESS" ] || [ "${#SAMPLES[@]}" -eq 0 ]; then
...
python input_to_geneModel.py -i "${INTEGRATION_DATA}" -g "${TRANSCRIPT_GTF}" -e "${EXON_GTF}" -r "${REF_FA}" -o "${OUTPUT_DIR}" -s "${SAMPLES[@]}"
...
# Run the R script
Rscript MCPV_geneModel.R -d "${OUTPUT_DIR}" -o "${OUTPUT_DIR}"/MCPyV_geneModel_plots/ -f "${FILE_TO_PROCESS}"
