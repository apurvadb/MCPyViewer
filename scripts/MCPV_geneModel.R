...
for (target_sample in unique(geneTrack_df$sample)) {
  for (target_gene in unique(geneTrack_df$gene)) {
    gene_subset <- geneTrack_df %>%
      filter(sample == target_sample, gene == target_gene)
    
    if (n_distinct(gene_subset$ins) > 1) {
      unique_genes <- gene_subset %>%
        distinct(ins, .keep_all = TRUE)
      
      for (k in seq_len(nrow(unique_genes))) {
        gene_data = unique_genes[k, ]
        chr <- gene_data$chr
    ...
    if (!is.na(chr)) {
      ...
      aaTrack <- AnnotationTrack(start = annotation_gene$start, width = annotation_gene$length,
                                 ...
                                 name = as.character(annotation_gene$sampleName[1]), 
                                 shape = 'box',
                                 col = NULL, 
